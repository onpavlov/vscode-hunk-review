...
- **`commentStore.ts`** — persists comments to `.hunk-review/comments.json` (store-and-forward: works before any hunk session exists) and moves committed ones to `.hunk-review/archive.json`. Comments carry `status: pending | sent | stale` and a `sessionCommentId` once applied.
...
- **Archiving after a commit**: `archiveAfterCommit` (called from `onWorktreeChanged`, before the stale check) compares `repo.state.HEAD.commit` with the last seen one. If the sha changed on the *same branch name* (a branch switch is excluded), every comment of any status whose anchor is no longer in the diff is moved by `CommentStore.archive` from `comments.json` to `.hunk-review/archive.json` (with `archivedAt` and the new `commit`); comments whose lines are still changed (partial commit) stay active. Line-number drift after a partial commit is not compensated.
...
- New user-facing commands must be declared in both `package.json` (`contributes.commands`) and `vscode.commands.registerCommand` in `src/extension.ts`, with matching `hunk-review.`-prefixed IDs. `activationEvents` is `onStartupFinished` so the status bar buttons appear without running a command first.
